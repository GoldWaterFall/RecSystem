from load_data import create_user_item_score
...
def predict_score_baseUser(uid,iid,user_item,sim_dict):
    ...
    #user_sim: {'36':1.0,'140':1.0,'147':1.0,'155':1.0.....}
...
def predict_click_baseUser(uid, user_item, sim_dict, top_item=200):
    ...
    for uid, value in uid_sim_otherid:
        item_set = set(user_item[uid]) - uid_item  # 过滤目标用户看过的物品
        rec_item = rec_item | item_set   # 取并集
...
if __name__ == '__main__':
    ...
    # 给用户推荐物品
    s = predict_all_score_baseUser("1", user_item, item_set, sim_dict, top_item=10)
    print(s)